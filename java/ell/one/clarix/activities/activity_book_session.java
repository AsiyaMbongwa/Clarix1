...
// Extract slot details
            String date = selectedSlotToBook.getString("date");
            String start = selectedSlotToBook.getString("startTime");
            String end = selectedSlotToBook.getString("endTime");
            String docId = selectedSlotToBook.getId();

// Sample price (replace with real logic if needed)
            double sessionPrice = selectedSlotToBook.contains("price") ? selectedSlotToBook.getDouble("price") : 500.0;
...
// Pass all required data
            ...
            intent.putExtra("price", sessionPrice);

// Add student info if available
            if (currentUser != null) {
                intent.putExtra("studentName", currentUser.getDisplayName() != null ? currentUser.getDisplayName() : "Student");
                intent.putExtra("studentEmail", currentUser.getEmail());
                intent.putExtra("studentPhone", currentUser.getPhoneNumber() != null ? currentUser.getPhoneNumber() : "N/A");
            }
                ...
                .addOnSuccessListener(docRef -> {
                    db.collection("users").document(tutorId)
                            ...
                            .addOnSuccessListener(unused -> {
                                ...
                                startActivity(new Intent(activity_book_session.this, activity_tutee_bookings.class));
                                finish();
                            })
                            .addOnFailureListener(e -> {
